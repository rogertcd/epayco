...
class encryption_class {
    ...
    //entradas
    var $padding;       // padding for filling the message


    function do_pad_tamano($texto_original){
        $tamano =  strlen('' . $texto_original );
        $wilcard = 	str_pad($tamano, 2, "0", STR_PAD_LEFT);
        return $wilcard . $texto_original;
    }

    function undo_pad_tamano($texto_casi_orig){
        $tam = substr($texto_casi_orig, 0,2);
        return substr($texto_casi_orig, 2,$tam);
    }
    ...
    function encryption_class ()
    {
        ...
        $this->scramble2 = 'AEIOU213579468BPFVDTSZGKCJHQLRMNWY0X';

        if (strlen($this->scramble1) <> strlen($this->scramble2)) {
            trigger_error('** SCRAMBLE1 is not same length as SCRAMBLE2 **', E_USER_ERROR);
        } // if

        $this->adj = 1.76;  // this value is added to the rolling fudgefactors
        ...
        // decrypt string into its original form
    {
        ...
        $target =  $this->undo_pad_tamano($target);
        ...
        // encrypt string into a garbled form
    {
        ...
        //$old = $source;
        $source = $this->do_pad_tamano($source);
        //echo '(' .$old  . ' -- '  . $source .')';
        ...
        $source = str_pad($source, $sourcelen, $this->padding);
        //echo $source . '<br/>';;
